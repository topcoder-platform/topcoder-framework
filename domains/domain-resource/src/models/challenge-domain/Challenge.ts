...
export interface UpdateChallengePhasesRequest {
...
function createBaseChallengePhase(): ChallengePhase {
  return {
    name: "",
    duration: 0,
...
export const ChallengePhase = {
  encode(
    ...
    if (message.scheduledStartTime !== undefined) {
      writer.uint32(26).string(message.scheduledStartTime);
    }
    if (message.scheduledEndTime !== undefined) {
      writer.uint32(34).string(message.scheduledEndTime);
    }
    if (message.actualStartTime !== undefined) {
      writer.uint32(42).string(message.actualStartTime);
    }
    if (message.actualEndTime !== undefined) {
      writer.uint32(50).string(message.actualEndTime);
    }
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): ChallengePhase {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  fromJSON(object: any): ChallengePhase {
    return {
      name: isSet(object.name) ? String(object.name) : "",
...
function createBaseUpdateChallengePhasesRequest(): UpdateChallengePhasesRequest {
  return { phases: [] };
}
...
export const UpdateChallengePhasesRequest = {
  ...
  decode(
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.phases.push(ChallengePhase.decode(reader, reader.uint32()));
          break;
        default:
          reader.skipType(tag & 7);
          break;
