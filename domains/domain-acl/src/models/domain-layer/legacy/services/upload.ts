...
export interface LegacyUploadClient extends Client {
  ...
  lookup(request: LookupCriteria, callback: (error: ServiceError | null, response: Upload) => void): ClientUnaryCall;
  lookup(
  ...
  delete(request: LookupCriteria, callback: (error: ServiceError | null, response: Empty) => void): ClientUnaryCall;
  delete(
    request: LookupCriteria,
    metadata: Metadata,
...
export const LegacyUploadClient = makeGenericClientConstructor(
  ...
  new (address: string, credentials: ChannelCredentials, options?: Partial<ClientOptions>): LegacyUploadClient;
